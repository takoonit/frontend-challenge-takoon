...
export async function fetchGeocoding(
	query: string
): Promise<{ results: GeocodingResponse[]; error: string | null }> {
	try {
		const response = await fetch(
			`${GEOCODING_API_URL}?text=${encodeURIComponent(query)}&apiKey=${GEOCODING_API_KEY}`
		);
		if (!response.ok) {
...
export async function fetchHourlyForecast(
	...
	try {
		...
		const result: HourlyForecast[] = data.list
			.slice(0, 8)
			.map((entry: any) => ({
				time: entry.dt * 1000,
